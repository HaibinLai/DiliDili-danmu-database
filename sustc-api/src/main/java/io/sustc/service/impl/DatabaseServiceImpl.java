...
import io.sustc.exception.CsvErrorException;
...
//        int proceedUser = 1;
//        for(int i=0;i<proceedUser;i++){
//            int finalI = i;
//            Future<?> UserRecord = executorService.submit
//                    (() -> InsertBiliUser(userRecords.subList(userRecords.size() * finalI /proceedUser,userRecords.size()* (finalI+1)/proceedUser)));
//        }
//
//        int proceedVideo = 1;
//        for(int i=0;i<proceedVideo;i++){
//            int finalI = i;
//            Future<?> VideoRecord = executorService.submit
//                    (() -> InsertVideoRecord(videoRecords.subList(videoRecords.size() * finalI /proceedVideo,videoRecords.size()* (finalI+1)/proceedVideo)));
//        }
//
...
//            throw new RuntimeException(e);
    ...
    public void truncate() {
        ...
        try (Connection conn = dataSource.getSQLConnection();
